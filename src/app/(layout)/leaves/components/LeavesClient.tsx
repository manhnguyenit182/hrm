...
export function LeavesClient({ initialLeaveRequests }: LeavesClientProps) {
  const { user } = useAuth();
  const [visible, setVisible] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);
  const [leaveRequests, setLeaveRequests] = useState<LeaveRequests[]>(initialLeaveRequests);
  ...
  useEffect(() => {
    if (canCreateLeave && leaveRequests.length > 0) {
