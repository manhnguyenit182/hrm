...
interface DepartmentsClientProps {
  initialDepartments: DepartmentWithEmployees[];
}

export function DepartmentsClient({ initialDepartments }: DepartmentsClientProps): React.JSX.Element {
  const router = useRouter();
  const [departments, setDepartments] = useState<DepartmentWithEmployees[]>(initialDepartments);
          ...
          {Array.from({ length: 3 }, (_, index) => (
            <div key={index} className="flex items-center space-x-3 p-3 rounded-lg">
              <div className="relative">
                <Skeleton shape="circle" size="2.5rem" />
                <Skeleton shape="circle" size="0.75rem" className="absolute -bottom-0.5 -right-0.5" />
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {loading
          ...
          : departments.map((department) => (
              <div key={department.id} className="card-modern overflow-hidden group">
                  ...
                  {department.Employees && department.Employees.length > 0 ? (
                    <div className="space-y-3">
                      {department.Employees.slice(0, 4).map((employee) => (
                        <div key={employee.id} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors group/employee">
                          <div className="relative">
                            <Avatar
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 p-2">
          <div className="space-y-2">
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <div>
                  <InputText id="name" {...field} placeholder="Nhập tên phòng ban..." className="w-full" />
                  {fieldState.error && (
          ...
          <div className="space-y-2">
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <InputText id="location" {...field} placeholder="Nhập địa điểm văn phòng..." className="w-full" />
              )}
            />
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            />
            <Button type="submit" label="Tạo phòng ban" className="btn-primary !px-6 !py-3" />
          </div>
        </form>
      </Dialog>
    </div>
